...
def compare(a, b):
    ...
    Args:
        a (int): first item to compare
        b (int): second item to compare

    Returns:
        int: smaller value of the two
    """
    if a <= b:
        return a
    else:
...
def mergeSorted(listA, listB):
    ...
    for i in range(length):
        # check if outside bounds of listA
        if i > len(listA):
            merged[i] = listB[i]
        # check if outside bounds of listB
        elif i > len(listB):
            merged[i] = listA[i]
        # otherwise compare items and add to list
        else:
            merged[i] = compare(listA[i], listB[i])
